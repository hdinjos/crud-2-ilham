<?php
    ...
    // Get Data
    $data_kategori = get_data("*", "id=$id", "kategori", $connect);
    ...
    if ( isset($_POST['ubah-data']) ) {
        ...
        $data = [
            "id"             => $connect->real_escape_string($_POST['id_kategori']),
            "nama"           => $connect->real_escape_string($_POST['nama_kategori']),
        ];

        $edit = edit_data("kategori", $data, $connect);
...
<main class="mt-5">
    <div class="container">
        <form action="edit_kategori.php" method="POST" class="form">
            <input type="number" class="form-control" name="id_kategori" hidden value="<?=$id?>">
